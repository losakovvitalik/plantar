...
- В репозитории нет CI (в `.github/workflows` только `release.yml`), поэтому `gh pr checks` на шаге 4
  отвечает `no checks reported`. Это не ошибка, но и не проверка: единственное подтверждение, что
  код собирается и тесты зелёные, — самоотчёт фиксера. Считать его гарантией нельзя; в сводке для
  пользователя пиши, что проверки прогонял фиксер, а не CI.
