...
- **next** — как node, но после установки зависимостей выполняется серверная сборка `buildCommand`; старый релиз остаётся рабочим, если сборка падает. Переменные проекта копируются в `.env` до сборки, затем приложение запускается через pm2 и проксируется nginx.
