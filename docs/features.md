...
- **Перенос переменных окружения** — при импорте env-файлы из папки приложения (`.env`, `.env.local`, `.env.prod` и т.п. — `listAppEnvFiles` / `readAppEnv` в `packages/core/src/discover.ts`; шаблоны вида `.env.example` и `.envrc` пропускаются) копируются в хранилище переменных Plantar одним текстом, от базового файла к переопределяющему (порядок dotenv). Дальше они видны в редакторе переменных и применяются при первом же деплое.
