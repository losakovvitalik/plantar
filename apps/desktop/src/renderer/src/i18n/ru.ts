...
export const ru = {
  ...
  "app.identityChangedNote":
    "По этому адресу отвечает сервер с другим опознавательным ключом. Обычно так бывает после переустановки сервера у хостинга, но точно так же выглядит и подмена. Пока ключ не совпадает, Plantar не подключается и ничего туда не отправляет. Если сервер переустанавливали — удалите его здесь и добавьте заново; вместе с сервером из списка пропадут и его проекты, добавить их придётся снова. Если нет — не подключайтесь и обратитесь в поддержку хостинга.",
