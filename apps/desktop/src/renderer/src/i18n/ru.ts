...
export const ru = {
  ...
  "app.identityChanged": "Сервер отвечает не так, как раньше.",
  "app.identityChangedNote":
    "По этому адресу отвечает сервер с другим опознавательным ключом. Обычно так бывает после переустановки сервера у хостинга, но точно так же выглядит и подмена. Пока ключ не совпадает, Plantar не подключается и ничего туда не отправляет. Если сервер переустанавливали — запомните его новый ключ кнопкой ниже, сервер и его проекты останутся на месте. Если нет — не подключайтесь и обратитесь в поддержку хостинга.",
  ...
  "app.externalBadgeHint":
    "Приложение работало на сервере до Plantar и обновляется бережно: деплой происходит в его папке на сервере, настройки веб-сервера не меняются.",
  ...
  "sidebar.status.server.identityChanged": "Сервер отвечает не так, как раньше",
  ...
  "deploy.deployedNoResponse":
    "Приложение обновлено, но по адресу {url} оно не ответило — подробности в логе деплоя.",
  "deploy.deployedPlainHttp":
    "Приложение обновлено, но по адресу {url} оно не ответило. Ответ пришёл по незащищённому адресу {plainUrl} — откройте его и посмотрите, ваше ли это приложение: точно так же отвечает и стандартная страница веб-сервера, если этот адрес на нём не настроен.",
  "deploy.openPlainUrl": "Открыть {url}",
  "deploy.botDeployed": "Бот задеплоен и запущен.",
  "deploy.deployedDone": "Приложение обновлено и запущено.",
  ...
  "deploy.rolledBackNoResponse":
    "Предыдущая версия возвращена, но по адресу {url} приложение не ответило — подробности в логе деплоя.",
  "deploy.rolledBackPlainHttp":
    "Предыдущая версия возвращена, но по адресу {url} приложение не ответило. Ответ пришёл по незащищённому адресу {plainUrl} — откройте его и посмотрите, ваше ли это приложение: точно так же отвечает и стандартная страница веб-сервера, если этот адрес на нём не настроен.",
  "deploy.rolledBackDone": "Предыдущая версия возвращена.",
  "deploy.externalHint":
    "Приложение импортировано с сервера и обновляется бережно: Plantar скачивает свежий код из репозитория прямо в папку приложения, устанавливает зависимости, собирает и перезапускает его под прежним именем. Настройки веб-сервера и другие приложения не затрагиваются.",
  "deploy.externalNoGitHint":
    "В папке приложения на сервере не нашлось репозитория с кодом, поэтому деплой и возврат версий недоступны: Plantar не может обновить код так, как это делалось бы вручную. Доступен перенос под управление Plantar.",
  "deploy.externalLinkHint":
    "Для переноса под управление Plantar сначала подключите репозиторий или укажите папку с кодом.",
  "deploy.migrate": "Перевести под управление Plantar",
  "deploy.migrateNeedsFolder":
    "Сначала подключите репозиторий или укажите папку с кодом.",
  "deploy.returnPrevious": "Вернуть предыдущую версию",
  "deploy.returnPreviousHint":
    "Приложение не поднялось после деплоя. Можно вернуть последнюю успешно развёрнутую версию — она будет собрана и запущена заново.",
  "deploy.returnPreviousConfirm":
    "Вернуть последнюю успешно развёрнутую версию? Это повторный деплой: с установкой зависимостей и сборкой.",
  ...
  "discover.hint":
    "Проект добавится как внешний и будет обновляться бережно: деплой выполняется в его папке на сервере, настройки веб-сервера не меняются.",
  ...
  "versions.banner":
    "Версии приложения по истории репозитория на сервере. Возврат версии — это повторный деплой выбранной версии с установкой зависимостей и сборкой, поэтому он занимает время.",
  "versions.behindTip": "Сейчас развёрнута не последняя версия ветки.",
  "versions.behindTipHint":
    "Так бывает после возврата версии: папка приложения закреплена на старой версии кода, и ручное обновление кода на сервере будет выдавать ошибку. Обычный деплой вернёт приложение на последнюю версию.",
  "versions.updatesAvailable": "В ветке появились новые изменения.",
  "versions.updatesAvailableHint":
    "Это обычная ситуация: обновление кода на сервере вручную по-прежнему работает. Обычный деплой развернёт последнюю версию.",
  "versions.loading": "Загружаю версии с сервера…",
  "versions.load": "Загрузить версии",
  "versions.passwordNeeded": "Понадобится пароль сервера.",
  "versions.refresh": "Обновить",
  "versions.empty": "Версий пока нет.",
  "versions.current": "Развёрнута",
  "versions.restore": "Вернуть эту версию",
  "versions.confirm":
    "Развернуть версию {hash}? Приложение будет пересобрано и перезапущено.",
  "versions.noGit":
    "В папке приложения на сервере нет репозитория с кодом, поэтому список версий недоступен.",

  "migrate.title": "Перевести приложение под управление Plantar?",
  "migrate.description":
    "Plantar пересоздаст приложение в своей структуре. Вот что изменится на сервере:",
  "migrate.itemPath":
    "Код переедет из {oldDir} в {newDir} — у каждой версии будет своя папка, возврат версии станет мгновенным.",
  "migrate.itemPm2":
    "Процесс будет пересоздан под именем «{name}»; прежний процесс «{pm2Name}» будет остановлен и убран из автозапуска.",
  "migrate.itemNginx":
    "Прежняя настройка веб-сервера ({file}) будет отключена — Plantar настроит свою.",
  "migrate.itemEnv":
    "Переменные из .env приложения переедут в хранилище Plantar и продолжат применяться при деплое.",
  "migrate.itemData":
    "Данные в старой папке приложения (базы данных, загруженные файлы) не переносятся. Если приложение хранит их рядом с кодом, позаботьтесь о переносе заранее.",
  "migrate.after":
    "После переноса проект станет обычным проектом Plantar: версии с мгновенным возвратом, управление доменом и HTTPS.",
  "migrate.confirm": "Перенести и задеплоить",
  ...
  "env.bannerExternal":
    "Переменные этого приложения читаются и сохраняются прямо в файл .env в его папке на сервере — отдельное хранилище не используется. Изменения применятся после перезапуска приложения, например при следующем деплое.",
  ...
  "history.openSiteNoResponse":
    "Сразу после деплоя по этому адресу ответа не было — подробности в логе.",
  "history.openSitePlainHttp":
    "Сразу после деплоя по защищённому адресу ответа не было, откроется незащищённый {url} — подробности в логе.",
  ...
  "projectSettings.domainExternal": "Адрес приложения",
  "projectSettings.domainExternalHint":
    "Адрес нужен только для того, чтобы после обновления проверить, отвечает ли приложение. Сам адрес на сервере Plantar не меняет: если приложение открывается по другому адресу или его нет совсем, поле можно исправить или оставить пустым. Включить для адреса защищённое соединение (HTTPS) можно на вкладке «Статус».",
  ...
  "removeProject.description":
    "Локальная папка проекта в любом случае останется на месте, а история деплоев и сохранённые логи этого проекта будут удалены.",
  ...
  "trustHostKey.action": "Сервер переустанавливали",
  "trustHostKey.title": "Запомнить новый ключ сервера «{name}»?",
  "trustHostKey.description":
    "Plantar снова начнёт подключаться к этому серверу и будет считать его прежним. Сервер и его проекты останутся в списке.",
  "trustHostKey.deployOnCommitNote":
    "У этого сервера настроен деплой при коммите ({projects}). В GitHub записан прежний ключ сервера, и деплой оттуда не пройдёт — после подтверждения настройте деплой при коммите заново.",
  "trustHostKey.warning":
    "Соглашайтесь, только если сервер действительно переустанавливали — вы сделали это сами или так сказал хостинг. Если ничего такого не было, по этому адресу может отвечать чужой сервер, и вы откроете ему доступ к своим приложениям и данным.",
  "trustHostKey.fingerprintLabel": "Ключ, которым сервер отвечает сейчас",
  "trustHostKey.fingerprintLabelTyped":
    "Ключ вида {type}, которым сервер отвечает сейчас",
  "trustHostKey.fingerprintHint":
    "Хостинг обычно показывает ключи сервера в панели управления — по одной строке на каждый вид. Сравните ключ выше со строкой того же вида: у строк другого вида значения не совпадут, и это нормально. Если значения совпадают — это ваш сервер. Если строки нужного вида в панели нет, уточните у хостинга, действительно ли сервер переустанавливали.",
  "trustHostKey.loading": "Смотрю ключ…",
  "trustHostKey.settled":
    "Сервер снова отвечает прежним ключом — запоминать нечего.",
  "trustHostKey.confirm": "Запомнить новый ключ",
  "trustHostKey.saving": "Сохраняю…",
  ...
  "appStatus.trafficSharedLog":
    "Приложение записывает посещения в общий журнал сервера — вперемешку с другими сайтами, поэтому отделить заходы именно на этот сайт невозможно. Собственный журнал появится, если перенести приложение под управление Plantar.",
  "appStatus.trafficSharedLogEnable":
    "Приложение записывает посещения в общий журнал сервера — вперемешку с другими сайтами, поэтому отделить заходы именно на этот сайт невозможно. Plantar может завести отдельный журнал: для этого в настройки веб-сервера приложения добавится одна строка.",
  "appStatus.trafficEnableLog": "Вести отдельный журнал…",
  ...
  "appStatus.behindTip": "Развёрнута не последняя версия.",
  "appStatus.behindTipNote":
    "Обычный деплой вернёт приложение на последнюю версию ветки.",
  "appStatus.behindTipHint":
    "После возврата версии папка приложения закреплена на старой версии кода — ручное обновление кода на сервере будет выдавать ошибку до следующего деплоя через Plantar.",
  "appStatus.httpsTitle": "Защищённое соединение (HTTPS)",
  "appStatus.httpsExternalNote":
    "Если сайт открывается по адресу с https:// и браузер не предупреждает о небезопасном соединении — всё уже настроено, ничего делать не нужно. Если защищённого соединения нет, Plantar может получить бесплатный сертификат и включить его, аккуратно дополнив существующие настройки веб-сервера.",
  "appStatus.httpsSetup": "Настроить HTTPS…",

  "httpsDialog.title": "Настроить HTTPS для {domain}",
  "httpsDialog.what":
    "На сервере запустится инструмент выпуска сертификатов (certbot). Он проверит, что адрес {domain} действительно ведёт на этот сервер, получит бесплатный сертификат Let's Encrypt и дополнит существующие настройки веб-сервера так, чтобы сайт открывался по защищённому соединению. Обычный адрес (http) начнёт перенаправлять на защищённый.",
  "httpsDialog.safety":
    "Сертификат будет продлеваться автоматически. Если выпустить его не получится, инструмент сам вернёт настройки в прежнее состояние — сайт продолжит работать как раньше.",
  "httpsDialog.confirm": "Настроить HTTPS",
  "httpsDialog.busy": "Настраиваю…",
  "httpsDialog.done":
    "HTTPS настроен: сайт открывается по защищённому адресу, сертификат будет продлеваться автоматически.",

  "visitsLog.title": "Вести отдельный журнал посещений",
  "visitsLog.what":
    "Эта строка добавится в каждую часть настроек веб-сервера, которая направляет запросы к приложению, — она указывает записывать посещения сайта в отдельный файл:",
  "visitsLog.file": "Файл настроек: {file}",
  "visitsLog.safety":
    "Перед изменением Plantar сохранит копию файла настроек и проверит результат; если проверка не пройдёт, прежний файл вернётся на место. Существующие строки не меняются.",
  "visitsLog.after":
    "Статистика начнёт накапливаться с этого момента — прежние посещения в неё не попадут.",
  "visitsLog.confirm": "Добавить журнал",
  "visitsLog.busy": "Добавляю…",
  ...
  "settings.mcpServer": "Доступ для ИИ-агентов",
  "settings.mcpServerHint":
    "ИИ-агенты на этом компьютере (например, Claude Code или Cursor) смогут читать сведения о ваших серверах и приложениях: состояние, логи, посещаемость, историю деплоев. Изменять что-либо они не смогут, пока это не разрешено отдельно.",
  "settings.mcpAllowDeploy": "Разрешить агентам деплой",
  "settings.mcpAllowDeployHint":
    "Агенты смогут запускать деплой и возвращать предыдущую версию приложений — это заменяет работающую версию. Читать сведения можно и без этого разрешения.",
  "settings.mcpEndpoint": "Адрес",
  "settings.mcpPortPending":
    "будет определён после сохранения настроек — обычный адрес занят другой программой.",
