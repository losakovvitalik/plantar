 ...
 *
 * The icons are tiny sprout PNGs embedded as base64 (see scripts history in
 * the PR): macOS gets a black template image (the system recolors it for the
 * menu bar), Windows/Linux get a green one. Tray look on Linux varies by
 * desktop environment — best effort.
 */
...
export function createAppTray(onOpenWindow: () => void): void {
  ...
  tray.on("click", () => {
    if (process.platform === "win32") openWindow?.();
  });
}
